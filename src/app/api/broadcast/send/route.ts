...
import { adminDb, adminMessaging } from '@/lib/firebase/admin-config';
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const { title, message, sendPush, sendInApp, sendEmail, adminId } = body;
        ...
        if (sendEmail) {
            // Extract emails of users who provided one
            ...
            if (emails.length > 0 && process.env.GMAIL_USER) {
                ...
                for (let i = 0; i < emails.length; i += chunkSize) {
                    ...
                    try {
                        ...
                        console.error("Email broadcast error:", e);
                    }
                }
            }
        }
