    ...
    const handleSendMobileOtp = async () => {
        ...
        try {
            const res = await fetch('/api/otp/send', {
                ...
                body: JSON.stringify({ phone: newMobileInput }),
            });
            const data = await res.json();
            if (data.success) {
                setMobileOtpSent(true);
                toast.success("OTP sent to " + newMobileInput);
            } else {
    ...
    const handleVerifyMobileOtp = async () => {
        ...
        try {
            const res = await fetch('/api/otp/verify', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone: newMobileInput, code: newMobileOtpCode }),
            });
            const data = await res.json();
            if (!data.success) {
                                            ...
                                            {!mobileOtpSent ? (
                                                ...
                                                <div className="space-y-3">
                                                    <div className="text-xs text-green-700 bg-green-50 rounded-lg p-2.5 border border-green-200">
                                                        Code sent to <strong>{newMobileInput}</strong>. Check your SMS.
                                                    </div>
                                                    <input
                                                        type="text"
                                                        inputMode="numeric"
